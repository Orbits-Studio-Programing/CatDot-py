...
def trandint(val1, val2, depth="TRANDTRUECODETYPExX1"): 
    import secrets
    low, high = min(val1, val2), max(val1, val2)
    range_size = (high - low) + 1
    if depth == "TRANDTRUECODETYPExX1":
        depth = secrets.SystemRandom().randint(3, 10)
    mix = secrets.SystemRandom().random()
    for _ in range(depth):
        mix = (mix * 9876.54321 + secrets.SystemRandom().random()) % 1.0
    return low + int(mix * range_size)

def trandom(depth="TRANDTRUECODETYPExX1"):
    import secrets
    seed_value = secrets.SystemRandom().random()
    if depth == "TRANDTRUECODETYPExX1":
        depth = random.randint(3, 10)
    for _ in range(depth):
        seed_value = (seed_value * 12345.6789 + secrets.SystemRandom().random()) % 1.0   
    return seed_value
